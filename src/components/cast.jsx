            ...
            cast && cast.map((person, index)=> {
                return (
                    ...
                    >
                        ...
                        {
                          (person?.character ?? '').length > 10 ? (person?.character ?? '').slice(0, 10) + '...' : (person?.character ?? 'person')
                        }
                        </Text>
                        <Text className="text-neutral-400 text-xs mt-1">
                        {
                          (person?.original_name ?? '').length > 10 ? (person?.original_name ?? '').slice(0, 10) + '...' : (person?.original_name ?? 'person')
                        }
                        </Text>
                    </TouchableOpacity>
                )
